...
p + ggspatial::geom_spatial_polygon(data=cdat,aes(x=x,y=y,fill=region,group=region),alpha=0.8) #May generate a harmless warning for leaving a stat_spatial_identity() argument default
...
ldat <- cnormed[p1 != p2][, data.table::data.table(
  region = rep(p1,2),
  x = c(x1,x2),
  y = c(y1,y2),
...
for(i in unique(ldat$region)){
  P <- p +
    ggspatial::geom_spatial_path(data=ldat[region==i],aes(x=y,y=x,alpha=count,size=count),colour=centres[region==i]$col,lineend="round") +
    ggspatial::geom_spatial_polygon(data=cdat[region==i],aes(x=x,y=y,group=region),fill=centres[region==i]$col) +
...
for(i in unique(ldat_top$region)){
  P <- P +
    ggspatial::geom_spatial_path(data=ldat_top[region==i],aes(x=y,y=x,alpha=count,size=count),lineend="round") +
